	...
	LPARAM lParam)    // second message parameter
{
	...
	case WM_KEYDOWN:
		...
		case VK_LEFT: // left arrow
			if (num2 == 0)
			{
				...
				HPEN pen5 = CreatePen(PS_SOLID, 5, RGB(0, 0, 0));
		...
		case VK_BACK: // backspace
			if (num == 0)
			{
				...
				paintObject(hWnd, hDC, ps, posX, posY, cursorPosition);
				paintPosition(hWnd, hDC, ps);
				HPEN pen2 = CreatePen(PS_SOLID, 5, RGB(0, 0, 0));
				SelectObject(hDC, pen2);
				SelectObject(hDC, GetStockObject(DC_BRUSH));
				SetDCBrushColor(hDC, RGB(0, 0, 255));
				Ellipse(hDC, 300, 300, 500, 100);
				EndPaint(hWnd, &ps);
				DeleteDC(hDC);
				num--;
			}
	...
	case WM_PAINT:
		...
		// 
		//rectangle
		HPEN pen1 = CreatePen(PS_SOLID, 10, RGB(0, 0, 0));
		SelectObject(hDC, pen1);
		Rectangle(hDC, 300, 300, 500, 500);

		SelectObject(hDC, GetStockObject(DC_BRUSH));
		SetDCBrushColor(hDC, RGB(0, 255, 0));
		Rectangle(hDC, 300, 300, 500, 100);

		SelectObject(hDC, GetStockObject(DC_BRUSH));
		SetDCBrushColor(hDC, RGB(255, 0, 0));
		Rectangle(hDC, 600, 600, 500, 50);

		POINT verticals[] = {{0,0}, {100,50}, {150,500}};
		Polygon(hDC, verticals, 3);
		EndPaint(hWnd, &ps);
		DeleteDC(hDC);

		
		HPEN pen3 = CreatePen(PS_SOLID, 5, RGB(0, 0, 0));
		...
		paintObject(hWnd, hDC, ps, posX, posY, cursorPosition);
		paintPosition(hWnd, hDC, ps);
		HPEN pen4 = CreatePen(PS_SOLID, 5, RGB(0, 0, 0));
